// src/pages/RegionPage.jsx — Tailwind CSS 4
//
// ROUTE: /regioni/:regionId  (protetta da autenticazione)
//
// FLUSSO DATI:
//   useParams()      → legge regionId dall'URL (es. "Toscana")
//   useAppStore      → sync region + filter (condivisi con Dashboard)
//   useTERNA(region) → dati reali TERNA/GSE (produzione GWh, capacità MW per fonte)
//   REGIONS          → coordinate dei capoluoghi per Open-Meteo
//   getRegionStats() → calcola totalMW, renewablePct, co2SavedTonnes da MW reali
//   useEnergyData()  → fetch irraggiamento solare per refCity della regione
//   SolarBarChart    → grafico 24h con dati reali Open-Meteo
...
export default function RegionPage() {
  ...
  // Dati reali TERNA per la regione + coordinate capoluogo
  ...
  // Sincronizza lo store con la regione dell'URL quando cambia
  useEffect(() => {
    if (refCity) setRegion(regionId);
  }, [regionId, refCity, setRegion]);

  // Coordinate del capoluogo per la chiamata API solare
  const { data: energyData, loading, error } = useEnergyData(
    refCity.lat,
    refCity.lng
  );

  // 404: regione non presente nel dataset
  if (!refCity) {
    return (
  ...
  // Impianti raggruppati per tipo, solo quelli con MW > 0
  const plantsByType = regionPlants.reduce((acc, p) => {
  ...
  // Filtrati per fonte (Tutti / Rinnovabili / Fossili)
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-fade-in">
      ...
      <div className={`rounded-2xl p-5 border
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          {/* Badge quota rinnovabile (verde se ≥70%, arancione altrimenti) */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          // CO2 evitata (migliaia di tonnellate)
      ...
      {/* ── Filtro impianti (Tutti / Rinnovabili / Fossili) ── */}
          ...
          filteredPlants.map((plant, i) => (
            <div key={plant.subtype}
              className={`grid grid-cols-[auto_1fr_auto_auto] gap-4 items-center px-5 py-3.5
                ...
                {/* Capacità (verde rinnovabile, arancione fossile) */}
      ...
      {/* ── Produzione reale TERNA per fonte (GWh) ── */}
      ...
      {/* ── Combustibili ed emissioni (dati reali) ── */}
      ...
      {/* ── Domanda elettrica (bilancio regionale) ── */}
